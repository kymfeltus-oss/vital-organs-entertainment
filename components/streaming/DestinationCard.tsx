...
export default function DestinationCard({ destination, onReload, onToast, onSettings, onEditSetup }: DestinationCardProps) {
  ...
  return (
    <>
      <div className="rounded-lg border border-white/8 bg-black/60 p-3">
        <div className="flex flex-wrap items-start justify-between gap-2">
          <div className="flex min-w-0 items-start gap-3">
            <PlatformLogo platform={String(platform)} className="h-9 w-9 text-xs" />
            <div className="min-w-0">
              <p className="font-body text-sm font-semibold text-white">{meta?.label ?? destination.destinationName}</p>
              {displayAccount ? (
                <p className="font-body text-xs text-white/60">{displayAccount}</p>
              ) : null}
              {isReady ? (
        ...
        {destination.lastErrorMessage && !isReady ? (
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {status === "not_connected" && meta?.oauth ? (
            <button type="button" disabled={busy} onClick={connectOAuth} className={TS.btnPrimary}>Connect</button>
          ) : null}
          {isConnected ? (
            <>
              <button type="button" disabled={busy} onClick={() => void runTest()} className={TS.btnBlue}>Test Connection</button>
              ...
              <button type="button" disabled={busy} onClick={() => onSettings(destination)} className={TS.btnOutline}>Settings</button>
              {onEditSetup ? (
                <button type="button" disabled={busy} onClick={onEditSetup} className={TS.btnOutline}>Edit Setup</button>
              ) : null}
              <button
                type="button"
                disabled={busy}
              ...
              {meta?.oauth ? (
                <button type="button" disabled={busy} onClick={connectOAuth} className={TS.btnOutline}>Change Account</button>
              ) : null}
            </>
          ) : null}
          {!meta?.oauth && status === "not_connected" ? (
            <button type="button" disabled={busy} onClick={() => onSettings(destination)} className={TS.btnPrimary}>Set Up</button>
          ) : null}
          <button
            type="button"
