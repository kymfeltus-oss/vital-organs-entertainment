...
export default function LiveReactionBar({
  ...
  return (
    <div className={`${compact ? "experience-reaction-tray shrink-0" : "experience-reaction-tray mb-2 shrink-0"}`}>
      {!compact ? (
      ...
      <div
        className="experience-reaction-tray flex min-w-0 flex-1 justify-between gap-1 sm:justify-start sm:gap-1.5"
        role="toolbar"
      ...
      >
        {LIVE_REACTION_CATALOG.map(({ type, label, emoji }) => (
          <button
            ...
            className="touch-target flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-lg transition active:scale-95 disabled:cursor-not-allowed disabled:opacity-40 sm:h-10 sm:w-10"
          >
