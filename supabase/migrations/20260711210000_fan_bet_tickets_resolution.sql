...
CREATE TABLE IF NOT EXISTS public.fan_bet_tickets (
...
CREATE INDEX IF NOT EXISTS fan_bet_tickets_open_pool_idx
...
CREATE TABLE IF NOT EXISTS public.liv_micro_bet_resolution_archive (
  ...
  total_tokens_distributed integer NOT NULL DEFAULT 0 CHECK (total_tokens_distributed >= 0),
...
CREATE UNIQUE INDEX IF NOT EXISTS liv_micro_bet_resolution_archive_room_bet_uidx
...
ALTER TABLE public.liv_micro_bet_resolution_archive ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.liv_micro_bet_resolution_archive FORCE ROW LEVEL SECURITY;

REVOKE ALL ON public.fan_bet_tickets FROM PUBLIC, anon, authenticated;
REVOKE ALL ON public.liv_micro_bet_resolution_archive FROM PUBLIC, anon, authenticated;
GRANT ALL ON public.fan_bet_tickets TO service_role;
GRANT ALL ON public.liv_micro_bet_resolution_archive TO service_role;

CREATE OR REPLACE FUNCTION public.resolve_and_payout_micro_bet(
  p_room_id text,
  p_bet_id text,
...
BEGIN
  ...
  LOOP
    ...
    IF v_ticket.selection = p_winning_option THEN
      PERFORM public.credit_seed_wallet(v_ticket.user_id, v_ticket.payout);
      ...
      VALUES (
        ...
        format('LIV micro-bet payout: %s → %s', p_bet_id, p_winning_option),
        v_ticket.id
      );
      ...
      v_total_paid := v_total_paid + v_ticket.payout;
    ELSE
      UPDATE public.fan_bet_tickets
      SET
        status = 'lost',
...
REVOKE ALL ON FUNCTION public.resolve_and_payout_micro_bet(text, text, text, uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.resolve_and_payout_micro_bet(text, text, text, uuid) TO service_role;
