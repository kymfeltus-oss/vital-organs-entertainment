...
import { connectMixer, loadTodaysService } from "@/lib/todays-service/service";
...
export async function POST(request: NextRequest) {
  return withServiceAuth(
    ...
    async (ctx) => {
      ...
      const result = await connectMixer(ctx.tenantId, body, ctx.user.id, ctx.user.email ?? null);
