import type { ColemanHomeSession, ChordBlock, VoicingPill, TheoryLike } from "./home-types";
...
function buildVoicings(chords: string[]): VoicingPill[] {
  if (chords.length < 2) return [{ id: "v-add", label: "[ + ]" }];
  const pairs: VoicingPill[] = [];
  for (let i = 0; i < Math.min(chords.length - 1, 4); i += 1) {
    pairs.push({ id: `v-${i}`, label: `[ ${chords[i]} / ${chords[i + 1]} ]` });
  }
  pairs.push({ id: "v-add", label: "[ + ]" });
  return pairs;
}

export function buildHomeSessionFromTheory(entry: TheoryLike): ColemanHomeSession {
  ...
  return {
    ...
    intelligence: {
      ...
      cadencePercent: 70,
      dialPrimary: root,
      dialSecondary: `${displayRoot} ${quality}`,
