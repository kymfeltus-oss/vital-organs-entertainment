...
const REDIS_KEY_PREFIX = "todays-service:sound:";

export type SoundLiveEvent = {
  tenantId: string;
...
const memoryListeners = new Map<string, Set<(event: SoundLiveEvent) => void>>();
...
export async function publishSoundLiveUpdate(event: SoundLiveEvent): Promise<void> {
...
export async function subscribeSoundLiveUpdates(
  tenantId: string,
  onMessage: (event: SoundLiveEvent) => void,
  signal?: AbortSignal,
): Promise<() => void> {
  ...
  const handler = (receivedChannel: string, message: string) => {
    ...
    try {
      onMessage(JSON.parse(message) as SoundLiveEvent);
    } catch {
      /* ignore */
    }
  };
